...
public class UserTagImportReduce  extends Reducer<Text, Text, Text,Text>  {
	...
	public void reduce(Text key,
		...
		for(Text t:values)
		{
			...
			if(v.length>1)
			{
	        	...
	        	if(tagMap!=null && !tagMap.isEmpty() && tagMap.containsKey(v[0]))
	        	{
					content.setAction("0");
					content.setContent(tagMap.get(v[0]));
					float f=0;
					try
					{
						 f = Float.parseFloat(v[1]);
					}
					catch(Exception e)
					{	
						f=0;
